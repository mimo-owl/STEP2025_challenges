...
Resizable and Collision-aware hash table implementation
output: ch1/outputs/out_hash_table2.txt
'''
# Hash function.
#
# |key|: string
...
def calculate_hash(key):
    assert type(key) == str
    hash = 0
    multiplier = len(key) - 1
    # 文字の登場順番を考慮してハッシュ値を計算（文字のASCIIコードを利用）
    for i, char in enumerate(key):
        hash += (26 ** multiplier * ord(char))
        multiplier -= 1
    return hash
...
class HashTable:
    ...
    # 再ハッシュ
    def resize(self, new_bucket_size):
        # print("resizing!")

        # 新しいハッシュを用意
        new_buckets = [None] * new_bucket_size

        # 元のハッシュマップからアイテムを新しいハッシュマップに移動
        for bucket in self.buckets:
            ...
            while item:
                ...
                # リフォーマット
                new_item = Item(item.key, item.value, new_buckets[new_index])
                new_buckets[new_index] = new_item
                # 同一bucket内の次のアイテムを見にいく
                item = item.next
    ...
    def put(self, key, value):
        ...
        if self.item_count >= self.bucket_size * 0.7:
            ...
            self.resize(new_bucket_size)
        elif self.item_count <= self.bucket_size * 0.3 and self.bucket_size > 1:
            # new_bucket_size = self.bucket_size // 2 + 1
            new_bucket_size = (self.bucket_size // 2) | 1
            self.resize(new_bucket_size)
    ...
    def delete(self, key):
        ...
        while item:
            if item.key == key:
                ...
                # それ以外の場合、前のアイテムのnextを現在のアイテムのnextに設定
                else:
                    prev.next = item.next
                self.item_count -= 1
                return True
